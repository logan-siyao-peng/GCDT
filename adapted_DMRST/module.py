...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_sentence, EDU_breaks, is_test=False):
        ...
        total_edu_loss = torch.FloatTensor([0.0])
        if USE_CUDA:
            total_edu_loss = total_edu_loss.cuda()
        ...
        for i in range(len(input_sentence)):
            ...
            bert_token_ids = torch.LongTensor(bert_token_ids)
            if USE_CUDA:
        ...
        for output in tem_outputs:
            ...
            zero_filling = torch.zeros(1, max_edu_break_num - cur_break_num, self.word_dim)
            if USE_CUDA:
                zero_filling = zero_filling.cuda()
            all_outputs.append(torch.cat([output, zero_filling], dim=1))
...
class Segmenter(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.loss_function = nn.CrossEntropyLoss(weight=weight_tensor)
    ...
    def train_segment_loss(self, word_embeddings, edu_breaks):
        ...
        edu_break_target = torch.LongTensor(edu_break_target)
        edu_start_target = torch.LongTensor(edu_start_target)
        if USE_CUDA:
            edu_break_target = edu_break_target.cuda()
            edu_start_target = edu_start_target.cuda()
